...
const {selectedProductByCate} = storeToRefs(productStore)


 
onMounted(() => {
  const id = route.params.id;
  productStore.getProductByCategory(Number(id));
});
...
watch(
  () => route.params.id,
  (newId) => {
    productStore.selectedProductByCate = []; // Đảm bảo xóa sạch danh sách trước khi gọi API
    productStore.getProductByCategory(Number(newId));
  }
);
...
<template>
    <div class="container">
      ...
      <div class="product-list">
        ...
        <div class="product-container" v-for="prod in selectedProductByCate" :key="prod.id">
